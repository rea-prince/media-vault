...
javac --module-path %PATH_TO_FX% --add-modules javafx.controls,javafx.fxml -d out --source-path src src/mediavault/Main.java
java --module-path %PATH_TO_FX% --add-modules javafx.controls,javafx.fxml -cp out mediavault.Main
```
...
javadoc --module-path "$PATH_TO_FX" --add-modules javafx.controls,javafx.fxml -d docs -sourcepath src -subpackages mediavault
```
